document.addEventListener("DOMContentLoaded", () => {
  ...
  function createProduct() {
    ...
    if (
      ...
      for (
        let i = 0;
        i < (productObject.count > 1 ? productObject.count : 1);
        i++
      ) {
  ...
  search.oninput = function () {
    ...
    for (let i = 0; i < productData.length; i++) {
      if (
        SearchMood === "products" &&
        productData[i].products.toLowerCase().includes(searchValue)
      ) {
        table += `
            <tr>
                <td>${i}</td>
        ...
        SearchMood === "category" &&
        productData[i].category.toLowerCase().includes(searchValue)
      ) {
        table += `
            <tr>
                <td>${i}</td>
  ...
  function cleanData() {
    productData = productData.filter(
      (product) => product.products && product.price && product.category,
    );
    localStorage.setItem("productStorage", JSON.stringify(productData));
    readData();
  }
